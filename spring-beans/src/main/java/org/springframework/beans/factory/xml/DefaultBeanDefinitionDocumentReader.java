...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	 ...
	 *
	 * 通过Spring IoC容器对载入的Bean定义Document解析可以看出，
	 * 我们使用Spring时，
	 * 在Spring配置文件中可以使用<Import>元素来导入IoC容器所需要的其他资源，
	 * Spring IoC容器在解析时会首先将指定导入的资源加载进容器中。
	 * 使用<Ailas>别名时，Spring IoC容器首先将别名元素所定义的别名注册到容器中。
	 * 对于既不是<Import>元素，又不是<Alias>元素的元素，
	 * 即Spring配置文件中普通的<Bean>元素的解析由BeanDefinitionParserDelegate类的parseBeanDefinitionElement方法来实现
	 */
